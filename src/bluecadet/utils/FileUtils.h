...
public:
	...
	typedef std::function<void(const cinder::fs::path & path)> FileCallback;
	...
	static void find(const cinder::fs::path & dir, FileCallback callback, const std::set<std::string> extensions = {}, const bool recursive = true, const bool lowercaseExtensions = true);

	//! Filename including extension. Avoids issues with compiler/IDE version discrepancies.
	static std::string getFilename(const cinder::fs::path & path);

	//! Extension starting at the last '.' (e.g. 'file.ext' will return '.ext'). Avoids issues with compiler/IDE version discrepancies.
	static std::string getExtension(const cinder::fs::path & path);
};
